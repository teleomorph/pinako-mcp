...
DATA MODEL
The tab tree is hierarchical: Window Groups → Windows → Tabs. The browser's own Tab Groups are mirrored as container nodes (type='tabgroup') that sit between a window and its member tabs — see BROWSER TAB GROUPS below.
...
BROWSER TAB GROUPS
The colored, named chips in the browser's own tab strip. The browser owns their title and color; Pinako mirrors them read-only. In the tree a Tab Group is a CONTAINER NODE: a real tree node with type='tabgroup', sitting directly under a window and holding its member tabs as children (tabs nested under a member are members too). Its title is the browser group's name (empty title is legal); its color is the browser's group color.
Membership is exactly containment: a tab is in the group if and only if it sits inside that branch. Member tabs also carry chromeGroupId / chromeGroupTitle / chromeGroupColor (include_chrome_tab_groups). Membership is controlled by move_node and nothing else — move a tab INTO or OUT OF the tabgroup node. You cannot create, rename, recolor, or dissolve a browser Tab Group — set_title and set_row_color reject tabgroup nodes. Never confuse browser Tab Groups with Window Groups (type='group') or Library Groups.
  ...
  srv.registerTool(
    'search_tabs',
    {
      ...
      inputSchema: {
        ...
        include_chrome_tab_groups: z.boolean().optional().describe('Include chromeGroupId + chromeGroupTitle + chromeGroupColor metadata on tab nodes (browser Tab Group, NOT Window Groups type="group"). type="tabgroup" container nodes also carry chromeGroupId + chromeGroupColor. Default true.'),
        include_star_color:        z.boolean().optional().describe('Include per-node starColor when set. Default true (cheap when absent).'),
  ...
  srv.registerTool(
    'list_libraries',
    {
      ...
      inputSchema: {
        ...
        include_chrome_tab_groups: z.boolean().optional().describe('Shape opt (when include_tabs:true): include chromeGroupId/Title/Color metadata on tab nodes (browser Tab Group, NOT Window Groups). type="tabgroup" container nodes also carry chromeGroupId + chromeGroupColor. Default true.'),
  ...
  srv.registerTool(
    'get_library',
    {
      ...
      inputSchema: {
        ...
        include_memos:             z.boolean().optional().describe('Include memoText per node. Default true.'),
        include_lineage:           z.boolean().optional().describe('Include the collapsed flag. Default true.'),
        include_chrome_tab_groups: z.boolean().optional().describe('Include chromeGroupId/Title/Color metadata on tab nodes (browser Tab Group, NOT Window Groups). type="tabgroup" container nodes also carry chromeGroupId + chromeGroupColor. Default true.'),
