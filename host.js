...
AUTO-ORGANIZE WORKFLOW (NATIVE CHAT ONLY)
There are no auto-organize MCP tools. Pinako's multi-step bookmark sorting workflow runs in the extension's native AI chat panel; the popup orchestrator drives the workflow deterministically with the LLM scoped to per-batch categorization decisions only. If the user asks to organize / reorganize / clean up / sort / categorize / auto-organize / tidy / structure their bookmarks: direct them to open the Pinako extension popup and use the native AI chat panel (Pro tier 1+), or click the auto-organize button on the bookmarks panel in Pinako. Do NOT attempt to plan a manual organization from get_bookmarks output via this MCP — large bookmark trees exceed agent context windows and the safety nets (silent undo manifest, Review folder, per-batch Pause/Resume, duplicate reconciliation, polish menu) only exist in the native chat workflow.
CONNECTION RECOVERY
